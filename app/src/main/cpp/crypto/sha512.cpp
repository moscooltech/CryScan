// Complete SHA-512 implementation (FIPS 180-4)
// Test vector: sha512("abc") = ddaf35a193617aba cc417349ae20413112e6fa4e89a97ea20a9eeee64b55d39a
...
static constexpr uint64_t H0_512[8] = {
...
inline uint64_t Ch64(uint64_t e, uint64_t f, uint64_t g)  { return (e & f) ^ (~e & g); }
inline uint64_t Maj64(uint64_t a, uint64_t b, uint64_t c) { return (a & b) ^ (a & c) ^ (b & c); }
...
inline uint64_t sigma0_512(uint64_t x) { return rotr64(x, 1)  ^ rotr64(x, 8)  ^ (x >> 7); }
inline uint64_t sigma1_512(uint64_t x) { return rotr64(x, 19) ^ rotr64(x, 61) ^ (x >> 6); }
...
struct Sha512State {
    ...
    Sha512State() {
        for (int i = 0; i < 8; ++i) h[i] = H0_512[i];
    }
    ...
    void processBlock(const uint8_t block[128]) {
        ...
        for (int i = 0; i < 80; ++i) {
            uint64_t T1 = hh + Sigma1_512(e) + Ch64(e, f, g) + K512[i] + W[i];
            uint64_t T2 = Sigma0_512(a) + Maj64(a, b, c);
            hh = g; g = f; f = e; e = d + T1;
            d = c; c = b; b = a; a = T1 + T2;
        }
...
std::array<uint8_t, 64> sha512(const uint8_t* data, size_t len) {
    ...
    // SHA-512 uses 128-bit length field (16 bytes)
    // Pad: 0x80, zeros, then 128-bit length in bits (big-endian)
    uint8_t block[256] = {};
    ...
    if (remainder < 112) {
        ...
        store_be64(block + 112, bitlen_hi);
        store_be64(block + 120, bitlen_lo);
        state.processBlock(block);
    } else {
        // Need two blocks
        store_be64(block + 240, bitlen_hi);
        store_be64(block + 248, bitlen_lo);
        state.processBlock(block);
        state.processBlock(block + 128);
    }
